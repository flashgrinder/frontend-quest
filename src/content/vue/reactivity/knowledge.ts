...
// price и count становятся зависимостями total`,
  ...
  },
  {
    ...
    whenToUse:
      'В обычном коде trigger вручную не вызывают. Исключение — специальные API вроде triggerRef для shallowRef. Понимание trigger нужно для объяснения того, почему изменение `.value` обновляет computed, watcher или компонент.',
    howItWorks:
...
const editor = markRaw(createEditor())`,
  ...
  },
  {
    ...
    readingTime: '4 мин',
    whatIsIt:
      'nextTick позволяет дождаться, когда Vue применит DOM-обновления после изменения реактивного состояния. Изменение state не всегда сразу отражается в DOM в той же строке кода. Vue группирует обновления для эффективности.',
    whenToUse:
...
listElement.value?.scrollIntoView()`,
  ...
  },
  {
    ...
    whenToUse:
      'Readonly полезен, когда нужно показать состояние, но запретить внешнюю мутацию. Это часто встречается в composables и публичных API. Изменения должны идти через контролируемые функции.',
    howItWorks:
