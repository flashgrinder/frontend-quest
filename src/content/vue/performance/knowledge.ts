...
const visibleProducts = computed(() =>
  ...
  },
  {
    ...
    howItWorks:
      '�?зменения reactive state ставят component update job в очередь. Дубликаты в очереди схлопываются. После текущего синхронного кода Vue применяет обновления, а nextTick позволяет дождаться этого момента.',
    whyImportant:
...
const openAndMeasure = async () => {
  ...
  },
  {
    ...
    whenToUse:
      'nextTick используют после изменения состояния, если нужно измерить элемент, проскроллить к новому блоку или синхронизироваться с DOM-библиотекой. Его не нужно добавлять после каждого ref change. �?збыточный nextTick усложняет код.',
    howItWorks:
...
watch([query, filters], ([nextQuery, nextFilters]) => {
  ...
  },
  {
    ...
    whenToUse:
      '�?спользуй shallowRef для chart/editor/map instances, больших immutable snapshots и objects, где важна только замена ссылки. markRaw подходит для class instances и SDK objects. Не стоит применять их к данным, которые UI должен глубоко отслеживать.',
    howItWorks:
...
onMounted(() => {
  ...
  },
  {
    ...
    whenToUse:
      'Key должен быть стабильным всегда, когда элемент участвует в diffing: списки, dynamic components, forms, transitions. �?ногда key намеренно меняют, чтобы сбросить компонент, но это должно быть осознанным действием.',
    howItWorks:
...
const afterChange = total.value // recalculated`,
  ...
  },
  {
    ...
    whenToUse:
      'Сначала стоит писать простой корректный код, а затем измерять. �?сключение — известные архитектурные решения с очевидным масштабом, например виртуализация для десятков тысяч строк. Даже тогда нужно сохранять читаемость.',
    howItWorks:
...
# 5. Keep the code understandable`,
  ...
  },
  {
    ...
    whenToUse:
      'Диагностика нужна перед любой серьёзной оптимизацией. Vue DevTools помогает смотреть component updates, Performance panel — main thread, Lighthouse — lab-аудит, Web Vitals — пользовательские метрики. �?нструмент выбирают по гипотезе.',
    howItWorks:
...
export const mountEditor = (element: HTMLElement) => {
  ...
  },
  {
    ...
    whenToUse:
      '�?спользуй v-once для статичных секций, которые не зависят от пользователя, locale, темы или live data. Если содержимое может измениться, лучше не применять v-once. Для частично статичных блоков Vue compiler уже делает много оптимизаций сам.',
    howItWorks:
  ...
  },
  {
    ...
    whenToUse:
      'Deep watch можно использовать для небольших объектов или быстрых эффектов. Для больших форм лучше отслеживать конкретные поля, использовать debounce, dirty flags или explicit save. �?наче autosave может превратиться в источник нагрузки.',
    howItWorks:
...
watch([() => form.title, () => form.description], debounce(saveDraft, 500))`,
  ...
  },
  {
    ...
    whenToUse:
      'customRef стоит рассмотреть для поиска, autosave, фильтров и сценариев, где частые изменения нужно сгладить. Для обычного состояния он избыточен. �?ногда проще использовать debounce в обработчике события.',
    howItWorks:
  ...
  },
  {
    ...
    howItWorks:
      'Браузер измеряет моменты отрисовки, layout shifts и задержку взаимодействий. �?нструменты собирают эти значения и показывают проблемы. Каждая метрика имеет свой класс решений.',
    whyImportant:
...
# High CLS -> layout reservation`,
  ...
  },
  {
    ...
    whenToUse:
      'Оптимизация INP нужна, когда ввод, клики или route transitions реагируют медленно. Нужно профилировать main thread, искать long tasks и сокращать работу в критическом path. �?ногда помогает debounce, worker, виртуализация или серверная фильтрация.',
    howItWorks:
...
<template>
  ...
  },
  {
    ...
    readingTime: '5 мин',
    whatIsIt:
      'CLS измеряет неожиданные сдвиги layout. �?зображения, iframe, рекламные блоки и динамический контент без зарезервированного места часто создают высокий CLS. Vue-код может быть правильным, но UX будет дёргаться.',
    whenToUse:
...
import { SearchIcon, UserIcon } from 'icon-library'`,
  ...
  },
  {
    ...
    whenToUse:
      'Этот принцип важен при добавлении memoization, кешей, lazy loading, custom refs и сложных abstractions. Если метрика не улучшается или UX становится хуже, оптимизация неудачна. �?ногда проще удалить оптимизацию.',
    howItWorks:
